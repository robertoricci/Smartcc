...
class TipoChapa(Base):
    ...
    preco = Column(Float, nullable=False)  # R$ por chapa
    cor = Column(String(50))
    acabamento = Column(String(50))
    ...
    # Relacionamentos
    pecas_projeto = relationship("PecaProjeto", back_populates="tipo_chapa")
    
    def descricao_completa(self):
        """Retorna descrição completa da chapa"""
        return f"{self.nome} ({int(self.comprimento)}×{int(self.largura)}×{int(self.espessura)}mm)"
...
class TipoFita(Base):
    ...
    preco_rolo = Column(Float, nullable=False)  # R$ por rolo
    cor = Column(String(50))
    material = Column(String(50))  # PVC, ABS, Melamínico, etc
    fornecedor = Column(String(100))
    observacoes = Column(String(500))
    ativo = Column(Boolean, default=True)
    criado_em = Column(DateTime, default=datetime.now)
    ...
    # Relacionamentos
    pecas_projeto = relationship("PecaProjeto", back_populates="tipo_fita")
    
    def descricao_completa(self):
        """Retorna descrição completa da fita"""
        return f"{self.nome} ({int(self.largura)}mm)"
...
class Projeto(Base):
    ...
    status = Column(String(50), default="Em Orçamento")
    
    # Custos do projeto
    valor_total = Column(Float, default=0.0)
    valor_chapas = Column(Float, default=0.0)
    valor_fitas = Column(Float, default=0.0)
...
class PecaProjeto(Base):
    ...
    tipo_fita_id = Column(Integer, ForeignKey('tipos_fita.id'))
    ...
    tipo_chapa = relationship("TipoChapa", back_populates="pecas_projeto")
    tipo_fita = relationship("TipoFita", back_populates="pecas_projeto")
...
class DatabaseManager:
    ...
    def excluir_banco(self):
        ...
        if os.path.exists(self.db_path):
            ...
            print(f"Banco de dados '{self.db_path}' excluído.")
