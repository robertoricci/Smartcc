...
def gerar_pdf_por_tipo(resultados, config_projeto):
    ...
    # ====================================================================
    # PÁGINA INICIAL - RESUMO DO PROJETO
    # ====================================================================
    
    pdf.setFont("Helvetica-Bold", 20)
    pdf.drawCentredString(largura_pagina / 2, altura_pagina - 50, 
                          config_projeto.get('nome', 'PROJETO DE CORTE'))
    ...
    pdf.drawString(50, y, f"Data: {pd.Timestamp.now().strftime('%d/%m/%Y')}")
    y -= 30
    
    pdf.setFont("Helvetica-Bold", 14)
    pdf.drawString(50, y, "RESUMO DO PROJETO")
    y -= 25
    
    pdf.setFont("Helvetica", 11)
    pdf.drawString(50, y, f"• Total de chapas: {total_chapas_geral}")
    y -= 20
    pdf.drawString(50, y, f"• Total de tipos diferentes: {len(resultados)}")
    y -= 20
    pdf.drawString(50, y, f"• Custo de chapas: R$ {custo_total_chapas:.2f}")
    y -= 20
    pdf.drawString(50, y, f"• Custo de fitas: R$ {custo_total_fitas:.2f}")
    y -= 25
    
    pdf.setFont("Helvetica-Bold", 14)
    pdf.drawString(50, y, f"CUSTO TOTAL: R$ {custo_total_projeto:.2f}")
    
    pdf.showPage()
    
    # ====================================================================
    # PÁGINAS POR TIPO DE CHAPA
    # ====================================================================
    ...
    for tipo_chapa_id, resultado in resultados.items():
        ...
        y = altura_pagina - 90
        pdf.setFont("Helvetica", 11)
        pdf.drawString(50, y, f"Dimensão: {int(tipo_chapa.comprimento)}×{int(tipo_chapa.largura)}×{int(tipo_chapa.espessura)}mm")
        y -= 20
        pdf.drawString(50, y, f"Quantidade: {len(chapas)} chapas")
        y -= 20
        pdf.drawString(50, y, f"Preço unitário: R$ {tipo_chapa.preco:.2f}")
        y -= 20
        pdf.drawString(50, y, f"Custo chapas: R$ {custo_chapas_tipo:.2f}")
        y -= 25
        ...
        if custos_fita:
            ...
            for fita_info in custos_fita.values():
                tipo_fita = fita_info['tipo_fita']
                pdf.drawString(60, y, f"• {tipo_fita.nome}: {fita_info['total_metros']:.2f}m ({fita_info['rolos']} rolos) - R$ {fita_info['custo']:.2f}")
                y -= 18
            y -= 10
        
        # Custo total do tipo
        pdf.setFont("Helvetica-Bold", 13)
        pdf.drawString(50, y, f"Custo Total deste Material: R$ {custo_total_tipo:.2f}")
        
        pdf.showPage()
        
        # Diagramas das chapas deste tipo
        for chapa in chapas:
            gerador = engine.GeradorDiagrama(chapa)
            ...
            pdf.drawImage(img_reader, x, y, width=nova_largura, height=nova_altura, preserveAspectRatio=True)
            
            # Rodapé
            pdf.setFont("Helvetica", 8)
            pdf.drawString(30, 20, f"{tipo_chapa.nome} - Chapa {chapa.numero} | Aproveitamento: {chapa.calcular_utilizacao():.1f}%")
    ...
    # ====================================================================
    # PÁGINA FINAL - RESUMO DETALHADO
    # ====================================================================
    
    pdf.setFont("Helvetica-Bold", 18)
    pdf.drawCentredString(largura_pagina / 2, altura_pagina - 50, "RESUMO FINAL DE MATERIAIS E CUSTOS")
    ...
    for tipo_chapa_id, resultado in resultados.items():
        ...
        pdf.setFont("Helvetica", 10)
        pdf.drawString(60, y, f"• {len(chapas)} chapas × R$ {tipo_chapa.preco:.2f} = R$ {custo_chapas:.2f}")
        y -= 18
        
        if custos_fita:
            for fita_info in custos_fita.values():
                pdf.drawString(60, y, f"• Fita {fita_info['tipo_fita'].nome}: R$ {fita_info['custo']:.2f}")
                y -= 18
        
        pdf.setFont("Helvetica-Bold", 11)
        pdf.drawString(60, y, f"Subtotal: R$ {custo_total_material:.2f}")
        y -= 30
    ...
    pdf.drawString(400, y - 15, f"R$ {custo_total_projeto:.2f}")
    
    pdf.setFont("Helvetica", 8)
    pdf.drawCentredString(largura_pagina / 2, 30, 
                         f"Corte Certo Pro - Gerado em {pd.Timestamp.now().strftime('%d/%m/%Y às %H:%M')}")
