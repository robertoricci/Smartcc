...
def salvar_projeto_completo(config_projeto, pecas_data, resultados, custo_total):
    ...
    try:
        ...
        # Criar projeto (sem custo_total pois não existe no modelo)
        novo_projeto = Projeto(
            nome=config_projeto.get('nome', 'Projeto Sem Nome'),
            cliente_id=cliente_id,
            descricao=f"Custo total: R$ {custo_total:.2f}",
...
def tela_projetos():
    ...
    for projeto in projetos:
        ...
        # Contar peças e calcular custo total
        ...
        # Somar custos estimados
        for chapa_id, pecas_grupo in pecas_por_chapa.items():
            tipo_chapa = session.query(TipoChapa).get(chapa_id)
            if tipo_chapa:
                # Estimativa simples: cada 4 peças = 1 chapa (pode ser refinado)
                num_chapas_estimado = max(1, len(pecas_grupo) // 4)
                custo_total += num_chapas_estimado * tipo_chapa.preco
        
        with st.expander(
            f"📂 {projeto.nome} - {cliente_nome} | R$ {custo_total:.2f} | {total_pecas} peças",
            expanded=False
        ):
            ...
            with col_info2:
                st.markdown(f"**Custo Estimado:** R$ {custo_total:.2f}")
