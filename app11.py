...
def tela_otimizador():
    ...
    with st.form("form_peca_otimizador", clear_on_submit=True):
        ...
        if fitas_disponiveis:
            ...
            # SEMPRE mostrar bordas, mas com mensagem se não tem fita
            st.caption("Selecione as bordas que receberão fita:")
            
            if tipo_fita_id is None:
                st.caption("⚠️ Selecione um tipo de fita acima para habilitar as bordas")
            
            col_b1, col_b2, col_b3, col_b4 = st.columns(4)
            
            # Desabilitar checkboxes se não tem fita selecionada
            disabled = (tipo_fita_id is None)
            
            with col_b1:
                fita_comp1 = st.checkbox("🔼 Superior", key="fita_comp1_otim", disabled=disabled)
            with col_b2:
                fita_comp2 = st.checkbox("🔽 Inferior", key="fita_comp2_otim", disabled=disabled)
            with col_b3:
                fita_larg1 = st.checkbox("◀️ Esquerda", key="fita_larg1_otim", disabled=disabled)
            with col_b4:
                fita_larg2 = st.checkbox("▶️ Direita", key="fita_larg2_otim", disabled=disabled)
            
            # Se não tem fita, forçar tudo como False
            if tipo_fita_id is None:
